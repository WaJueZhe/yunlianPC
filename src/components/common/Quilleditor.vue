...
export default {
  ...
  methods: {
    ...
    onFileChange(e) {
      ...
      var filetype = fileInput.files[0].type.split("/").splice(-1);
      var self = this;
      var data = new FormData();
      data.append("file", fileInput.files[0], filetype);
      this.editor.focus();
      let config = {
        headers: { "Content-Type": "multipart/form-data" }
      }; //添加请求头
      this.axios.post(`${this.uploadUrl}`, data, config).then(res => {
        //console.log(res);
        if (res.data.success) {
          let url = this.api.resource + res.data.data.resulturl;
