<template>
  <el-row class="warp">
    ...
    <el-table :data="tableData" style="width:100%;" highlight-current-row v-loading="loading" stripe :border="true">
      ...
      <el-table-column prop="signerId"  label="签名人姓名" align="center" min-width="16%"></el-table-column>
...
export default {
  data() {
    return {
      ...
      loading: false,
      verifyManagementTitle: "签名管理",
      exceptionStatusEnum: [
        { consumeType: "", lbl: "全部" },
        { consumeType: "1", lbl: "签发" },
        { consumeType: "2", lbl: "起运" },
        { consumeType: "3", lbl: "签收" },
        { consumeType: "4", lbl: "异常单签收" },
        { consumeType: "5", lbl: "异常单确认" },
        { consumeType: "99", lbl: "充值" }
      ],
      exceptionStatusMold: [{ customerOrgId: "", lbl: "全部" }],
      filters: {
        customOrgId: "",
        consumeType: "",
        referId: "",
  ...
  methods: {
    ...
    handleExport() {
      ...
      that.axios
        .post(
          `${this.api.baseURL}/bsConsumeSign/queryConsumeSignExport`,
          params
        )
        .then(res => {
    ...
    search: function() {
      ...
      //console.log("提交数据 = "+ JSON.stringify(params))
      that.axios
        .post(`${this.api.baseURL}/bsConsumeSign/queryConsumeSign`, params)
        .then(res => {
          ...
          // alert(res.data.data.bsChargeBalance)
          ...
          that.isSuperAdmin =
            res.data.data.isSuperAdmin == 1 || res.data.data.isSuperAdmin == 2;
        });
    }
  },
  mounted(vm) {
